...
export default async function EvidenceGuideDetail({params}:{params:Params}){
 ...
 const [related]=await Promise.all([getRelatedEvidenceGuides(record)]); const refs=safeEvidenceReferences(record.references_json); const category=evidenceGuideCategory(record); const canonical=record.canonical_url||`/evidence-guides/${slug}/`; const url=`${SITE_URL}${canonical}`; const review=contentReviewProvenance(record);
