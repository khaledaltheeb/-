...
export default async function EvidenceGuidesPage(){
 ...
 return <><SiteHeader/><main className={styles.shell}><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema).replace(/</g,'\\u003c')}}/><nav className="breadcrumbs" aria-label="مسار الصفحة"><Link href="/">الرئيسية</Link><span>/</span><span aria-current="page">الأدلة المبنية على المصادر</span></nav><section className={styles.hero}><span className="eyebrow">مكتبة عملية قابلة للتدقيق</span><h1>{hub?.title||'الأدلة العربية المبنية على المصادر'}</h1><p>{hub?.excerpt||'أدلة عربية عملية تربط الفهم بالمصادر الأصلية وحدود الاستخدام.'}</p><div className={styles.stats}><strong>{items.length}</strong><span>دليلًا منشورًا · مصادر أصلية · مسارات محفوظة</span></div><form className={styles.search} action="/search" method="get" role="search"><label className="sr-only" htmlFor="evidence-guides-search">البحث في الأدلة العلمية</label><input id="evidence-guides-search" name="q" type="search" placeholder="ابحث عن موضوع أو سؤال داخل مكتبة روافد" maxLength={160}/><button type="submit">بحث</button></form><nav className={styles.localNav} aria-label="التنقل من مكتبة الأدلة"><Link href="/care-guides/">أدلة الرعاية</Link><Link href="/encyclopedia/">الموسوعة النفسية</Link><Link href="/sections">الأقسام</Link><Link href="/sectors/pediatric-oncology">سرطان الأطفال</Link></nav></section><section className={styles.guideIntro}><span className="eyebrow">طريقة الاستخدام</span><h2>كيف تستخدم هذه المكتبة؟</h2><p>ابدأ بالمشكلة أو السؤال الأقرب لاحتياجك. افحص الفروق المهمة، مؤشرات التقييم، الخطوات منخفضة المخاطر وحدود الدليل، ثم ارجع إلى المصادر الأصلية المرفقة. هذه الصفحات للتثقيف ولا تحول المعلومات العامة إلى تشخيص أو خطة علاج فردية.</p></section>{[...grouped.entries()].map(([category,rows])=><section className={styles.group} key={category}><div className={styles.groupHead}><div><span className="eyebrow">مجموعة أدلة</span><h2>{category}</h2></div><span>{rows.length} دليل</span></div><div className={styles.grid}>{rows.map((item)=><article className={styles.card} key={item.id}><div className={styles.cardMeta}><span>{item.references_json?.length??0} مصادر</span><span>{category}</span></div><h3><Link href={item.canonical_url||`/evidence-guides/${item.slug.replace(/^evidence-guides-/,'')}/`}>{item.title}</Link></h3>{item.excerpt&&<p>{item.excerpt}</p>}<Link className={styles.read} href={item.canonical_url||'#'}>فتح الدليل ←</Link></article>)}</div></section>)}</main><SiteFooter/></>;
}
