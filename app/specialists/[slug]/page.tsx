    ...
    <>
      ...
      <main className="profile-shell">
        ...
        <div className="profile-layout">
          ...
          <aside className="profile-sidebar"><div className="contact-card"><h2>التواصل</h2><p>تظهر وسائل الاتصال المباشرة فقط عندما يسمح صاحب الملف بعرضها.</p><div className="contact-actions"><Link className="primary-link" href={`/login?next=${encodeURIComponent(`/specialists/${specialist.slug}`)}`}>مراسلة داخل روافد</Link>{email && <a className="button" href={`mailto:${email}`}>البريد الإلكتروني</a>}{phone && <a className="button" href={`tel:${phone}`}>اتصال</a>}{website && <a className="button" href={website} target="_blank" rel="noopener noreferrer">الموقع الإلكتروني</a>}{mapUrl && <a className="button" href={mapUrl} target="_blank" rel="noopener noreferrer">عرض الموقع</a>}</div></div><div className="trust-card"><strong>حالة التوثيق</strong><span>موثق في منصة روافد</span>{specialist.verified_at && <small>آخر توثيق: {new Intl.DateTimeFormat('ar',{dateStyle:'medium'}).format(new Date(specialist.verified_at))}</small>}</div></aside>
        </div>
      </main>
      <SiteFooter />
    </>
