...
const pageHref = (page: number, filters: DirectoryFilters) => {
  ...
  return `/specialists${params.size ? `?${params.toString()}` : ''}`;
};
...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  return buildSeoMetadata({
    title: state.hasFilters ? 'نتائج تصفية دليل المختصين' : state.page > 1 ? `دليل المختصين النفسيين — الصفحة ${state.page}` : 'دليل المختصين النفسيين',
...
export default async function SpecialistsDirectory({ searchParams }: { searchParams: SearchParams }) {
  ...
  const directoryPath = pageHref(page, filters);
  const directoryUrl = `${SITE_URL}${directoryPath}`;
  ...
  const collectionSchema = {
    ...
    url: directoryUrl,
    name: hasFilters ? 'نتائج تصفية دليل المختصين' : page > 1 ? `دليل المختصين — الصفحة ${page}` : 'دليل المختصين',
