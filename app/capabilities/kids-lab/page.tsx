...
export const metadata = buildSeoMetadata({
  ...
  description: 'مكتبة عربية متدرجة لأنشطة الأطفال القابلة للطباعة: 67 سلسلة مهارية، خمسة مستويات، تدريب واختبار إتقان لكل مستوى، مع تصنيف حسب العمر والمهارة والاحتياج.',
...
export default function KidsLabPage() {
  ...
  const collection = {
    ...
    description: 'سلاسل مهارية متدرجة للأطفال، لكل مستوى تدريب واختبار إتقان قابلان للمعاينة والطباعة.',
  };
      ...
      <main className={styles.shell}>
        ...
        <section className={styles.hero}>
          ...
          <p className={styles.lead}>
            ليس مستودع أوراق عشوائيًا. كل مهارة تتحول إلى سلسلة من خمسة مستويات، وكل مستوى يحتوي تدريبين ثم اختبار إتقان مستقل. التصميم موجه للطفل: ألوان واضحة، مساحة عمل كبيرة، تعليمات قصيرة، ومعاينة كاملة قبل الطباعة.
          </p>
        ...
        <section className={styles.section}>
          <div className={styles.sectionHead}>
            <div><h2>كيف يعمل التدرج؟</h2><p>لا ينتقل الطفل لأن عمره ازداد أو لأنه أنهى الورقة، بل لأن أداءه في اختبار المستوى يدل على جاهزية معقولة للمهمة التالية.</p></div>
          </div>
        ...
        <section className={styles.section}>
          <div className={styles.launchCard}>
            <div>
              <span className={styles.kicker}>السلسلة الأولى جاهزة كأساس الإنتاج</span>
            ...
            <div className={styles.launchVisual} aria-hidden="true">
              ...
              <div className={`${styles.track} ${styles.trackBlue}`} />
            </div>
          </div>
        </section>
        ...
        <section className={styles.section}>
          <div className={styles.sectionHead}>
            <div>
              <h2>خريطة السلاسل الـ67</h2>
              <p>كل بطاقة أدناه تمثل مجالًا مهاريًا. السلسلة رقم 43 مفعلة في الدفعة التأسيسية، وبقية السلاسل مثبتة في الكتالوج لتنتج بنفس العقد التصميمي والعلمي.</p>
            </div>
          </div>
            ...
            {kidsLabCategories.map((category) => (
              <article className={styles.categoryCard} style={{ '--category-color': category.color } as React.CSSProperties} key={category.slug}>
                ...
                <div className={styles.seriesList}>
                  {category.series.map((series) => series.slug === 'bilateral-tracks' ? (
                    <Link href="/capabilities/kids-lab/bilateral-tracks/" className={styles.seriesActive} key={series.slug}>
                      ...
                      <span><strong>{series.title}</strong><small>{series.example}</small></span>
                      <span className={styles.readyBadge}>بدأ الإنتاج</span>
                    </Link>
                  ) : (
                    <div className={styles.seriesItem} key={series.slug}>
                      <span className={styles.seriesNumber}>{series.number}</span>
                      <span><strong>{series.title}</strong><small>{series.example}</small></span>
                      <span className={styles.badge}>{series.ages}</span>
                    </div>
                  ))}
                </div>
              </article>
        ...
        <section className={styles.section}>
          <div className={styles.note}>
            <strong>قاعدة علمية ثابتة:</strong> اختبارات المستوى هنا تقيس إتقان المهمة داخل السلسلة، ولا تُستخدم لتشخيص ADHD أو التوحد أو اضطراب تعلم أو أي حالة طبية/نفسية. يمكن تكييف النشاط لاحتياجات مختلفة، لكن اسم التشخيص وحده لا يحدد الورقة المناسبة.
          </div>
        </section>
      </main>
      <SiteFooter />
