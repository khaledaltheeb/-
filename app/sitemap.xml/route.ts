...
function inFilter(values: string[]) { return `(${values.join(',')})`; }
...
function releasePreviewSitemap(path?: string | null, lastModified?: string | null) {
  ...
  const row = url ? `<url><loc>${escapeXml(url)}</loc>${lastModified ? `<lastmod>${new Date(lastModified).toISOString()}</lastmod>` : ''}</url>` : '';
  const xml = `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${row}</urlset>`;
  return new NextResponse(xml, { headers: { 'Content-Type': 'application/xml; charset=utf-8', 'Cache-Control': 'private, no-store, max-age=0' } });
}

function applyDedicatedSitemapExclusions<T extends { not: (column: string, operator: string, value: string) => T; neq: (column: string, value: string) => T; }>(query: T): T {
  let owned = query
  ...
  for (const canonical of ATLAS_OWNED_CANONICALS) owned = owned.neq('canonical_url', canonical);
  return owned;
}
...
export async function GET(request: Request) {
  ...
  let encyclopediaQuery = supabase.from('content').select('id', { count: 'exact', head: true }).in('content_type', ['glossary_term', 'condition']).like('canonical_url', '/encyclopedia/%').eq('status', 'published').lte('published_at', now).eq('robots_index', true);
  if (releasedSlugs.length > 0) encyclopediaQuery = encyclopediaQuery.not('slug', 'in', inFilter(releasedSlugs));
  let contentQuery = supabase.from('content').select('id', { count: 'exact', head: true }).eq('status', 'published').lte('published_at', now).eq('robots_index', true);
  ...
  const [contentResult, quickInfoResult, encyclopediaResult] = await Promise.all([
    contentQuery,
    supabase.from('content').select('id', { count: 'exact', head: true }).like('canonical_url', '/quick-info/%').eq('status', 'published').lte('published_at', now).eq('robots_index', true),
    encyclopediaQuery,
  ]);
  ...
  const paths = [
    '/sitemaps/static.xml', '/sitemaps/daily-tools.xml', '/sitemaps/taxonomy.xml', '/sitemaps/cognitive-lab.xml', '/sitemaps/addiction-atlas.xml', '/sitemaps/social-work.xml', '/sitemaps/palliative-care.xml', '/sitemaps/rare-disease.xml', '/sitemaps/sightsavers-accessibility.xml', '/sitemaps/specialists.xml', '/sitemaps/centers.xml', '/sitemaps/community.xml',
