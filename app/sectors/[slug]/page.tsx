...
function sectorKeywords(sector: Sector) {
  return [...new Set([sector.name_ar, ...metadataTerms(sector.metadata, 'seo_keywords'), ...metadataTerms(sector.metadata, 'semantic_terms'), 'منصة روافد'])].slice(0, 40);
}
...
export async function generateMetadata({ params, searchParams }: { params: Params; searchParams: SearchParams }): Promise<Metadata> {
  ...
  return buildSeoMetadata({ title: page > 1 ? `${baseTitle} - الصفحة ${page}` : baseTitle, description: page > 1 ? `${baseDescription} صفحة ${page} من محتوى القطاع.` : baseDescription, path: pagePath(sector.slug, page), index: true, follow: true, keywords: sectorKeywords(sector) });
}
...
export default async function SectorPage({ params, searchParams }: { params: Params; searchParams: SearchParams }) {
  ...
  const [{ data: categories }, editorialResult] = await Promise.all([
    ...
    sector.editorial_content_id ? supabase.from('content').select('id,title,excerpt,body_json,body_text').eq('id', sector.editorial_content_id).eq('status', 'published').lte('published_at', now).maybeSingle() : Promise.resolve({ data: null }),
  ]);
  ...
  const collectionSchema = { '@context': 'https://schema.org', '@type': 'CollectionPage', '@id': `${canonicalUrl}#collection`, url: canonicalUrl, name: page > 1 ? `${sector.name_ar} - الصفحة ${page}` : sector.name_ar, description: sector.description || undefined, keywords: keywords.join(', '), inLanguage: 'ar', isAccessibleForFree: true, isPartOf: { '@id': `${SITE_URL}/#website` }, mainEntity: { '@type': 'ItemList', numberOfItems: totalContent, itemListElement: contentRows.map((item, index) => ({ '@type': 'ListItem', position: (page - 1) * PAGE_SIZE + index + 1, name: item.title, url: `${SITE_URL}${publicContentHref(item)}` })) } };
  ...
  return <><SiteHeader /><main className="site-shell sector-page" style={accentStyle}>
    ...
    <nav className="sector-quick-nav" aria-label={`وصول سريع داخل ${sector.name_ar}`}>{sector.slug === 'addiction-recovery' && <Link href="/addiction/">موسوعة الإدمان التفاعلية</Link>}{sector.slug === 'addiction-recovery' && <Link href="/addiction/substances/">أطلس المواد</Link>}{sector.slug === 'addiction-recovery' && <Link href="/addiction/compare/">المقارنات</Link>}{sector.slug === 'addiction-recovery' && <Link href="/addiction/interactions/">التفاعلات</Link>}{sector.slug === 'addiction-recovery' && <Link href="/addiction/prevalence/">الانتشار</Link>}{sector.slug === 'addiction-recovery' && <Link href="/addiction/mortality/">الوفيات</Link>}{sector.slug === 'addiction-recovery' && <Link href="/addiction/methodology/">المنهجية</Link>}<a href="#sector-categories">الأقسام الرئيسية</a>{totalContent > 0 && <a href="#sector-content">كل محتوى القطاع</a>}<Link href={`/search?q=${sectorQuery}`}>البحث في القطاع</Link><Link href="/care-guides/">أدلة التعامل والرعاية</Link><Link href="/evidence-guides/">الأدلة العلمية</Link>{sector.slug === 'pediatric-oncology' && <Link href="/search?q=سرطان+الأطفال+دعم+الأسرة">دعم أسرة طفل السرطان</Link>}</nav>
    ...
    <section className="section" id="sector-categories"><div className="section-mini-heading"><div><span className="eyebrow">موضوعات القطاع</span><h2>الأقسام الرئيسية</h2></div><span>{categoryRows.length.toLocaleString('ar')} قسمًا إجمالًا</span></div><div className="category-public-grid">{roots.map((category) => { const children = categoryRows.filter((candidate) => candidate.parent_id === category.id); return <article className="public-category-card" key={category.id}><Link href={`/sections/${category.slug}`}><h3>{category.name_ar}</h3></Link><p>{category.description || 'قسم متخصص ضمن هذا القطاع.'}</p>{children.length > 0 && <div className="subcategories">{children.map((child) => <Link href={`/sections/${child.slug}`} key={child.id}>{child.name_ar}</Link>)}</div>}<Link href={`/sections/${category.slug}`}>استعراض القسم ←</Link></article>; })}{!roots.length && <div className="empty-state"><strong>لا توجد أقسام عامة متاحة في هذا القطاع حاليًا.</strong></div>}</div></section>
