...
export default async function SectionPage({ params, searchParams }: { params: Params; searchParams: SearchParams }) {
  ...
  return <>
    ...
    <main className="site-shell sector-page">
      ...
      <section className="section related-content-section" id="section-content"><div className="section-heading"><span>المحتوى المنشور</span><h2>{query ? `نتائج البحث عن «${query}»` : `محتوى ${category.name_ar}`}</h2><p>صفحات مرتبة ضمن هذا القسم وفق التصنيف الأساسي المعتمد، وتفتح على عناوينها العامة الأصلية.</p></div>{rows.length ? <><div className="related-content-grid">{rows.map((item) => { const href = publicContentHref(item); return <article key={item.id}><span className="content-type-pill">{publicContentTypeLabel(item.content_type)}</span><h3><Link href={href}>{item.title}</Link></h3>{item.excerpt && <p>{item.excerpt}</p>}<Link href={href}>قراءة الصفحة ←</Link></article>; })}</div>{pages > 1 && <nav className="section-pagination" aria-label="صفحات نتائج القسم">{page > 1 && <Link href={pageHref(slug, page - 1, query)} rel="prev">الصفحة السابقة</Link>}<span>الصفحة {page.toLocaleString('ar')} من {pages.toLocaleString('ar')}</span>{page < pages && <Link href={pageHref(slug, page + 1, query)} rel="next">الصفحة التالية</Link>}</nav>}</> : <div className="empty-state"><strong>لا توجد نتيجة مطابقة.</strong><span>جرّب مصطلحًا أقصر أو انتقل إلى أحد الموضوعات الفرعية.</span></div>}</section>
    </main>
    <SiteFooter />
  </>;
}
