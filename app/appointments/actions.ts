...
function dateValue(fd:FormData,key:string){const raw=text(fd,key,80);if(!raw)return null;const date=new Date(raw);return Number.isFinite(date.getTime())?date.toISOString():null}
...
export async function requestAppointment(formData:FormData){
 ...
 const startsAt=dateValue(formData,'starts_at');if(!startsAt)redirect('/appointments?error=invalid-time');
...
export async function providerUpdateAppointment(formData:FormData){
 ...
 const supabase=await client('/appointments');const startsAt=dateValue(formData,'starts_at');const endsAt=dateValue(formData,'ends_at');
