...
export default async function AppointmentsPage({searchParams}:{searchParams:SearchParams}){
 ...
 return <><SiteHeader/><main className="communication-shell"><section className="communication-heading"><div><span className="eyebrow">Appointments Portal</span><h1>المواعيد</h1><p>طلباتك ومواعيدك كمستخدم أو كمقدم خدمة في مكان واحد، مع انتقالات حالة محكومة ومسجلة.</p></div><div className="communication-actions"><Link className="button" href="/messages">الرسائل</Link><Link className="button" href="/notifications">الإشعارات</Link></div></section>
 {params.created&&<div className="system-message success">تم إرسال طلب الموعد. سيبقى قيد الطلب حتى تؤكده الجهة.</div>}{params.cancelled&&<div className="system-message success">تم إلغاء الموعد وتسجيل العملية.</div>}{params.updated&&<div className="system-message success">تم تحديث الموعد وإشعار الطرف الآخر.</div>}{params.error&&<div className="system-message error">تعذر تنفيذ عملية الموعد. لم تُحفظ حالة غير صالحة.</div>}{error&&<div className="search-state error"><h2>تعذر تحميل المواعيد</h2></div>}
 <section className="appointment-section"><div className="section-mini-heading"><div><span className="eyebrow">Active</span><h2>القادمة وقيد التنسيق</h2></div><span>{upcoming.length}</span></div>{!error&&upcoming.length===0&&<div className="search-state"><h3>لا توجد مواعيد نشطة</h3><p>يمكن طلب موعد من ملف مختص أو مركز موثق.</p></div>}<div className="appointment-list">{upcoming.map(row=><AppointmentCard row={row} key={row.appointment_id}/>)}</div></section>
 {history.length>0&&<section className="appointment-section"><div className="section-mini-heading"><div><span className="eyebrow">History</span><h2>السجل</h2></div><span>{history.length}</span></div><div className="appointment-list">{history.map(row=><AppointmentCard row={row} key={row.appointment_id}/>)}</div></section>}
 </main><SiteFooter/></>;
}
...
function AppointmentCard({row}:{row:Appointment}){
 ...
 <div className="appointment-actions">{row.conversation_id&&<Link className="button" href={`/messages/${row.conversation_id}`}>فتح المحادثة</Link>}{row.counterpart_slug&&<Link className="button" href={row.specialist_id?`/specialists/${row.counterpart_slug}`:`/centers/${row.counterpart_slug}`}>الملف العام</Link>}
