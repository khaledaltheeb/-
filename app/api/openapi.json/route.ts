...
export async function GET(request: Request) {
  ...
  const document = {
    ...
    info: {
      ...
      description: 'واجهة عامة للمواد المنشورة فقط، مع سجل مصادر مترابط واكتشاف أدلة عبر Europe PMC وCrossref وDataCite وLens، وفصل منظم لحقوق metadata/content وprovenance للترجمة. مفاتيح الشركاء اختيارية وتضيف تعريفًا مؤسسيًا وحصص استخدام؛ ولا تمنح الواجهة تلقائيًا حق إعادة نشر النصوص أو المصادر الخارجية.',
    ...
    paths: {
      ...
      '/sources/{id}': { get: { tags: ['Sources'], operationId: 'getSource', security: partnerSecurity, parameters: [{ name: 'id', in: 'path', required: true, schema: { type: 'string', format: 'uuid' } }], responses: { '200': { description: 'Source metadata including related identifiers, ORCID/ROR connections, structured rights, translation provenance, versions and citations', content: { 'application/json': { schema: { $ref: '#/components/schemas/SourceDetailResponse' } } } }, '400': { $ref: '#/components/responses/BadRequest' }, '404': { $ref: '#/components/responses/NotFound' }, ...partnerResponses } } },
    ...
    components: {
      ...
      schemas: {
        ...
        SourceDetailResponse: { type: 'object', properties: { data: { type: 'object', properties: { related_identifiers: { type: 'array', items: { $ref: '#/components/schemas/RelatedIdentifier' } }, contributors: { type: 'array', items: { type: 'object' } }, organizations: { type: 'array', items: { type: 'object' } }, rights_profiles: { type: 'array', items: { $ref: '#/components/schemas/SourceRightsProfile' } }, translations: { type: 'array', items: { $ref: '#/components/schemas/SourceTranslationProvenance' } }, versions: { type: 'array', items: { type: 'object' } }, cited_by: { type: 'array', items: { type: 'object' } } } }, meta: { type: 'object' } } },
