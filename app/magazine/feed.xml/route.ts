...
export async function GET(request: Request) {
  ...
  const lastBuild = items[0]?.updated_at || items[0]?.published_at || null;
  const buildDate = lastBuild ? `<lastBuildDate>${escapeXml(new Date(lastBuild).toUTCString())}</lastBuildDate>` : '';
  const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom"><channel><title>المجلة والأبحاث | منصة روافد</title><link>${escapeXml(`${SITE_URL}/magazine/`)}</link><description>قراءات عربية نقدية للأبحاث والدراسات الحديثة.</description><language>ar</language>${buildDate}<atom:link href="${escapeXml(`${SITE_URL}/magazine/feed.xml`)}" rel="self" type="application/rss+xml"/>${items.map((item) => {
