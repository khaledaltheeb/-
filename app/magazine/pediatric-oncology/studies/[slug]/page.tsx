...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> {
  ...
  const record = await getPediatricOncologyEvidenceRecordForRequest('studies', slug, token);
  if (!record) return {};
...
export default async function PediatricOncologyStudyPage({ params, searchParams }: Props) {
  const [{ slug }, query] = await Promise.all([params, searchParams]);
  const token = releaseToken(query);
  const record = await getPediatricOncologyEvidenceRecordForRequest('studies', slug, token);
  if (!record) notFound();
