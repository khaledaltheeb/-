...
export async function GET(request: Request) {
  ...
  if (page === 0) {
    ...
    rows.push(`<url><loc>${escapeXml(absolute(QUICK_INFO_HUB.path))}</loc>${latest ? `<lastmod>${escapeXml(new Date(latest).toISOString())}</lastmod>` : ''}<changefreq>weekly</changefreq><priority>0.9</priority></url>`);
  }
  ...
  for (const item of items) {
    ...
    rows.push(`<url><loc>${escapeXml(absolute(item.canonicalUrl))}</loc>${item.updatedAt ? `<lastmod>${escapeXml(new Date(item.updatedAt).toISOString())}</lastmod>` : ''}<changefreq>monthly</changefreq><priority>0.8</priority><image:image><image:loc>${escapeXml(discoverUrl)}</image:loc></image:image><image:image><image:loc>${escapeXml(cardUrl)}</image:loc></image:image></url>`);
  }
