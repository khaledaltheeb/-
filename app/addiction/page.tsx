...
export async function generateMetadata(): Promise<Metadata> {
  ...
  // The addiction hub remains a published, crawlable review surface but must not
  // enter the index until its current scientific/editorial release gate is closed.
  // The preservation smoke contract intentionally protects this YMYL boundary.
  const canIndex = false;
  ...
  if (canIndex) metadata.robots = `index, ${record.robots_follow ? 'follow' : 'nofollow'}, max-image-preview:large, max-snippet:-1, max-video-preview:-1`;
  return metadata;
}
