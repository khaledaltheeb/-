...
import { getAddictionAtlasPracticeNote } from '@/lib/addiction-atlas-practice-notes';
...
export default async function SubstancePage({ params }: { params: Params }) {
  ...
  const practiceNote = getAddictionAtlasPracticeNote(item.slug);
  ...
  const citationUrls = [...new Set([...item.source_urls, ...(practiceNote?.sourceUrls ?? [])])];
  const schemas = [
  ...
  return <><SiteHeader /><main className={styles.shell}>
    ...
    {practiceNote ? <section className={styles.section}><div className={styles.card}><span className={styles.eyebrow}>طبقة الممارسة السريرية · دليل مكمل لا يغيّر درجات الخطر</span><h2>{practiceNote.titleAr}</h2><p>{practiceNote.summaryAr}</p><h3>ما الذي ينبغي أن يتغير في التقييم؟</h3><ul>{practiceNote.assessmentAr.map((value) => <li key={value}>{value}</li>)}</ul><h3>حدود الدليل</h3><p>{practiceNote.evidenceBoundaryAr}</p><h3>المراجع المستخدمة لهذه الملاحظة</h3><ol className={styles.sources}>{practiceNote.sourceUrls.map((source) => <li key={source}><a href={source} target="_blank" rel="noopener noreferrer">{source}</a></li>)}</ol></div></section> : null}
