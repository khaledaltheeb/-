...
export async function generateMetadata(): Promise<Metadata> {
  return buildSeoMetadata({
    ...
    description: 'أطلس عربي تفاعلي موثق للمواد ذات الصلة بالإدمان: الأسماء العربية والإنجليزية، ثمانية محاور للمخاطر، التفاعلات المراجعة، الانتشار، الوفيات، الانسحاب، الطوارئ، العلاج والمقارنات.',
    path: '/addiction/substances/', index: true, follow: true, type: 'website',
    keywords: ['أطلس المخدرات', 'المواد المسببة للإدمان', 'أضرار المخدرات', 'تفاعلات المخدرات', 'الجرعة الزائدة', 'انسحاب المخدرات', 'علاج الإدمان'],
    relatedTerms: ['opioids', 'stimulants', 'benzodiazepines', 'cannabis', 'hallucinogens', 'inhalants', 'drug interactions'],
  });
}
...
export default async function AddictionSubstancesPage() {
  ...
  return <><SiteHeader /><main className={styles.shell}>
    ...
    <header className={styles.hero}><span className={styles.eyebrow}>تمت المراجعة من قبل فريق روافد · 8 محاور مستقلة · عدم اليقين ظاهر</span><h1>الأطلس العربي التفاعلي للمواد والإدمان</h1><p>مكتبة منظمة تضم {atlas.substances.length} مادة أو عائلة دوائية/نفسية مع أسماء عربية وإنجليزية، معلومات سريرية موجزة، مخاطر متعددة المحاور، تفاعلات عالية الأهمية تمت مراجعتها، وطبقات مستقلة للانتشار والوفيات. لا تختزل الخطورة في رقم واحد ولا تعتبر القيم الأقل دليلاً على الأمان.</p><div className={styles.actions}><Link href="/addiction/compare/">{indexableComparisons.length} مقارنة تحريرية</Link><Link href="/addiction/interactions/">التفاعلات المراجعة</Link><Link href="/addiction/prevalence/">الانتشار والإحصاءات</Link><Link href="/addiction/mortality/">الوفيات والجرعات الزائدة</Link><Link href="/addiction/methodology/">اقرأ المنهجية</Link><Link href="/addiction/withdrawal-safety/">سلامة الانسحاب والطوارئ</Link></div></header>
