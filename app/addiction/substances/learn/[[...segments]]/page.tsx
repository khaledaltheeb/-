...
const routeFor = (segments: string[] = []) => `/addiction/substances/learn/${segments.filter(Boolean).join('/')}${segments.length ? '/' : ''}`;
...
export default async function AddictionLegacyLearnPage({ params }: { params: Params }) {
  const { segments = [] } = await params;
  const route = routeFor(segments);
