...
export default async function SpecialistContentEditor({params,searchParams}:{params:Params;searchParams:SearchParams}){
  ...
  return <main className="dashboard-shell"><section className="dashboard-card cms-editor">
    <div className="admin-heading"><div><span className="eyebrow">محتوى المختص</span><h1>{record.title}</h1><div className="editor-meta"><span className={`status-badge status-${record.status}`}>{STATUS_LABELS[record.status]??record.status}</span><span>آخر تعديل: {new Intl.DateTimeFormat('ar',{dateStyle:'medium',timeStyle:'short'}).format(new Date(record.updated_at))}</span></div></div><div className="dashboard-actions"><Link className="button" href="/specialist/media">مكتبتي الإعلامية</Link><Link className="button" href="/specialist/content">محتواي</Link>{record.status==='published'&&<Link className="button" href={`/content/${record.slug}`}>عرض الصفحة</Link>}</div></div>
