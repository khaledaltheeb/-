...
const pageHref = (page: number, filters: DirectoryFilters) => {
  ...
  return `/centers${params.size ? `?${params.toString()}` : ''}`;
};
...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  return buildSeoMetadata({
    title: state.hasFilters ? 'نتائج تصفية دليل المراكز' : state.page > 1 ? `دليل المراكز النفسية — الصفحة ${state.page}` : 'دليل المراكز النفسية',
...
export default async function CentersDirectory({ searchParams }: { searchParams: SearchParams }) {
  ...
  const collectionSchema = {
    ...
    url: directoryUrl,
    name: hasFilters ? 'نتائج تصفية دليل المراكز' : page > 1 ? `دليل المراكز — الصفحة ${page}` : 'دليل المراكز',
