...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  return buildSeoMetadata({
    ...
    description: 'فهرس منظم لكل صفحات قاعدة معرفة روافد المنشورة والقابلة للفهرسة، مع ربط كل صفحة بمسارها العام وتصنيفها الأساسي.',
...
export default async function AllPagesIndex({ searchParams }: { searchParams: SearchParams }) {
  ...
  const currentPath = pageHref(page, query);
  const schema = {
  ...
  return <>
    ...
    <main className="site-shell sector-page-shell all-pages-shell">
      ...
      <section className="public-index-hero" aria-labelledby="all-pages-title">
        ...
        <p>طبقة وصول مستقلة عن ترتيب القطاعات والأقسام، تعرض صفحات قاعدة المعرفة المنشورة والقابلة للفهرسة من دون تغيير مساراتها أو ملكيتها أو حالة فهرستها. الصفحات المحفوظة أو غير المفهرسة تبقى في مساراتها الحالية ولا تُكشف قسرًا عبر هذا الفهرس.</p>
        <div className="public-stat-strip"><span>{total.toLocaleString('ar')} {query ? 'نتيجة مطابقة' : 'صفحة معرفة قابلة للفهرسة'}</span><span>{totalPages.toLocaleString('ar')} صفحات فهرس</span><span>24 عنصرًا في الصفحة</span></div>
        ...
        {query && <p><Link href="/all-pages">مسح البحث والعودة إلى الفهرس الكامل ←</Link></p>}
      </section>
      ...
      <nav className="sector-quick-nav" aria-label="مسارات فهرس المحتوى">
        ...
        <Link href="/#labs-tools">المختبرات والتقييمات والأدوات</Link>
      ...
      <section className="section related-content-section" aria-labelledby="content-index-title">
        <div className="section-heading"><span>قاعدة المعرفة</span><h2 id="content-index-title">{query ? `نتائج «${query}»` : `الصفحة ${page.toLocaleString('ar')} من الفهرس`}</h2><p>كل بطاقة ترتبط بالمسار العام الفعلي للصفحة وتعرض القطاع والقسم الأساسيين عندما يكون التصنيف متاحًا.</p></div>
        {error ? <div className="search-state error"><h2>تعذر تحميل فهرس المحتوى</h2><p>لم تُعرض نتائج جزئية على أنها فهرس كامل. حاول مرة أخرى لاحقًا.</p></div> : rows.length > 0 ? <>
          ...
          <PublicPagination currentPage={paginationPage} totalPages={totalPages} hrefForPage={(targetPage) => pageHref(targetPage, query)} ariaLabel="صفحات فهرس المحتوى المنشور" />
        </> : <div className="empty-state"><strong>{query ? 'لا توجد صفحات مطابقة للبحث.' : total > 0 ? 'هذه الصفحة خارج نطاق الفهرس الحالي.' : 'لا توجد سجلات محتوى منشورة وقابلة للفهرسة حاليًا.'}</strong><span>{query ? 'جرّب عبارة أقصر أو انتقل إلى دليل الأقسام.' : total > 0 ? 'استخدم أرقام الصفحات للعودة إلى المحتوى المنشور.' : 'سيظهر المحتوى هنا فور نشره وفهرسته.'}</span>{total > 0 && <PublicPagination currentPage={paginationPage} totalPages={totalPages} hrefForPage={(targetPage) => pageHref(targetPage, query)} ariaLabel="صفحات فهرس المحتوى المنشور" />}</div>}
      </section>
    </main>
    <SiteFooter />
  </>;
