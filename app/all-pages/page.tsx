...
export async function generateMetadata({ searchParams }: { searchParams: SearchParams }): Promise<Metadata> {
  ...
  return buildSeoMetadata({
    title: query ? `نتائج فهرس الصفحات: ${query}` : page > 1 ? `فهرس الصفحات المنشورة — الصفحة ${page}` : 'فهرس الصفحات المنشورة',
    description: query
      ? `نتائج البحث داخل فهرس الصفحات المنشورة في منصة روافد عن ${query}.`
      : 'فهرس شامل للصفحات المنشورة في منصة روافد، مستقل عن بنية القطاعات والأقسام لضمان سهولة الوصول وعدم فقد المحتوى أثناء التوسع.',
...
export default async function AllPagesIndex({ searchParams }: { searchParams: SearchParams }) {
  ...
  const breadcrumbs = breadcrumbJsonLd([{ name: 'الرئيسية', path: '/' }, { name: 'فهرس الصفحات', path: '/all-pages' }]);
  ...
  const schema = {
    ...
    name: query ? `نتائج فهرس الصفحات عن ${query}` : 'فهرس الصفحات المنشورة في منصة روافد',
    inLanguage: 'ar',
  ...
  return <>
    ...
    <main className="site-shell sector-page-shell">
      ...
      <nav className="breadcrumbs" aria-label="مسار الصفحة"><Link href="/">الرئيسية</Link><span>/</span><span aria-current="page">فهرس الصفحات</span></nav>
      ...
      <section className="public-index-hero" aria-labelledby="all-pages-title">
        ...
        <h1 id="all-pages-title">كل الصفحات المنشورة</h1>
        <p>فهرس داخلي شامل للمحتوى المنشور، مستقل عن ترتيب القطاعات والأقسام. وجود هذه البوابة يضمن بقاء الصفحة قابلة للاكتشاف داخل المنصة حتى أثناء إعادة التصنيف أو توسع عدد الصفحات.</p>
        <div className="public-stat-strip"><span>{total.toLocaleString('ar')} {query ? 'نتيجة مطابقة' : 'صفحة منشورة'}</span><span>{pages.toLocaleString('ar')} صفحات فهرس</span><span>الوصول الداخلي مستقل عن قرار فهرسة محركات البحث</span></div>
        <form className="sector-search" action="/all-pages" method="get" role="search"><label className="sr-only" htmlFor="all-pages-search">البحث في الصفحات المنشورة</label><input id="all-pages-search" name="q" defaultValue={query} placeholder="ابحث بعنوان الصفحة أو وصفها" maxLength={120} /><button type="submit">بحث</button></form>
        {query && <Link className="content-index-reset" href="/all-pages">عرض الفهرس الكامل ←</Link>}
      </section>

      <nav className="sector-quick-nav" aria-label="مسارات فهرس الصفحات">
      ...
      <section className="section related-content-section" aria-labelledby="content-index-title">
        <div className="section-heading"><span>المحتوى المنشور</span><h2 id="content-index-title">{query ? `نتائج «${query}»` : `الصفحة ${page.toLocaleString('ar')} من الفهرس`}</h2><p>كل بطاقة ترتبط بالمسار العام الأصلي للصفحة، وتعرض موضعها التصنيفي عندما يكون متاحًا.</p></div>
        {rows.length > 0 ? <>
          ...
          <PublicPagination currentPage={Math.min(page, pages)} totalPages={pages} hrefForPage={(targetPage) => pageHref(targetPage, query)} ariaLabel="صفحات فهرس المحتوى المنشور" />
        </> : <div className="empty-state"><strong>{query ? 'لا توجد صفحات مطابقة للبحث.' : 'هذه الصفحة خارج نطاق الفهرس الحالي.'}</strong><span>{query ? 'جرّب عبارة أقصر أو انتقل إلى دليل الأقسام.' : 'استخدم أرقام الصفحات للعودة إلى المحتوى المنشور.'}</span><PublicPagination currentPage={Math.min(page, pages)} totalPages={pages} hrefForPage={(targetPage) => pageHref(targetPage, query)} ariaLabel="صفحات فهرس المحتوى المنشور" /></div>}
      </section>
    </main>
    <SiteFooter />
  </>;
