...
type BlockType = 'paragraph' | 'heading' | 'list' | 'quote' | 'callout' | 'table' | 'resource' | 'divider';
type EditorBlock = {
  id: string;
  type: BlockType;
  text?: string;
...
const labels: Record<BlockType, string> = {
  paragraph: 'فقرة', heading: 'عنوان فرعي', list: 'قائمة', quote: 'اقتباس', callout: 'تنبيه/معلومة', table: 'جدول', resource: 'مصدر خارجي', divider: 'فاصل',
};
...
function normalizeBlock(value: unknown): EditorBlock | null {
  ...
  if (type === 'callout') {
    const tone = ['info','success','warning','danger'].includes(toText(block.tone)) ? toText(block.tone) as EditorBlock['tone'] : 'info';
    return { id: uid(), type, title: toText(block.title), text: toText(block.text), tone };
  }
  if (type === 'table') {
    const rows = Array.isArray(block.rows) ? block.rows.map((row) => toStrings(row)) : [];
    return { id: uid(), type, caption: toText(block.caption), headers: toStrings(block.headers), rows };
  }
...
export default function BlockEditor({ bodyJson, bodyText }: Props) {
  ...
  const serialized = useMemo(() => JSON.stringify({ version: 2, format: 'blocks', blocks: blocks.map(clean) }), [blocks]);
  ...
  return <section className="block-editor" aria-labelledby="block-editor-title">
    ...
    <input type="hidden" name="body_text" value={plain} />
    <div className="block-editor-heading"><div><h2 id="block-editor-title">محتوى الصفحة</h2><p>ابنِ الصفحة من وحدات منظمة. لا HTML ولا JSON يدوي.</p></div><span>{blocks.length} وحدة</span></div>
    ...
    <div className="block-editor-list">
      {blocks.map((block, index) => <article className="editor-block" key={block.id}>
        ...
        {block.type === 'callout' && <div className="block-stack"><div className="block-inline"><select value={block.tone ?? 'info'} onChange={(event) => patch(block.id,{ tone:event.target.value as EditorBlock['tone'] })}><option value="info">معلومة</option><option value="success">نصيحة</option><option value="warning">تنبيه</option><option value="danger">تحذير</option></select><input value={block.title ?? ''} onChange={(event) => patch(block.id,{ title:event.target.value })} maxLength={300} placeholder="عنوان التنبيه" /></div><textarea rows={4} value={block.text ?? ''} onChange={(event) => patch(block.id,{ text:event.target.value })} maxLength={7000} placeholder="النص" /></div>}
