...
export default async function EditContentPage({params,searchParams}:{params:Params;searchParams:SearchParams}){
 ...
 return <main className="dashboard-shell"><section className="dashboard-card cms-editor">
  <div className="admin-heading"><div><span className="eyebrow">Versioned Content</span><h1>{record.title}</h1><div className="editor-meta"><span className={`status-badge status-${record.status}`}>{STATUS_LABELS[record.status]??record.status}</span>{isRevision&&<span className="status-badge">Revision غير عام</span>}<span>آخر تعديل: {new Intl.DateTimeFormat('ar',{dateStyle:'medium',timeStyle:'short'}).format(new Date(record.updated_at))}</span>{record.scheduled_at&&<span>موعد النشر: {new Intl.DateTimeFormat('ar',{dateStyle:'medium',timeStyle:'short'}).format(new Date(record.scheduled_at))}</span>}{record.published_at&&<span>النشر: {new Intl.DateTimeFormat('ar',{dateStyle:'medium',timeStyle:'short'}).format(new Date(record.published_at))}</span>}</div></div><div className="dashboard-actions"><Link className="button" href="/admin/content">كل المحتوى</Link>{fullAdmin&&!isRevision&&<Link className="button" href={`/admin/content/${record.id}/relations`}>الأقسام والوسوم</Link>}{record.status==='published'&&!isRevision&&<Link className="button" href={publicHref}>عرض الصفحة</Link>}</div></div>
