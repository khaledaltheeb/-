...
export async function createDraft(formData: FormData) {
  const supabase = await requireAdmin(); const data = buildContentPayload(formData); if (!data) redirect('/admin/content/new?error=invalid-input');
...
export async function updateDraft(formData: FormData) {
  const supabase = await requireAdmin(); const id = field(formData, 'id', 60); const data = buildContentPayload(formData); if (!validUuid(id) || !data) redirect('/admin/content?error=invalid-input');
...
export async function transitionStatus(formData: FormData) {
  const supabase = await requireAdmin(); const id = field(formData, 'id', 60); const target = field(formData, 'target', 40); const slug = field(formData, 'slug', 140);
