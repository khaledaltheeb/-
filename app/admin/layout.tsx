...
export default async function AdminLayout({ children }: Readonly<{ children: React.ReactNode }>) {
 ...
 return <div className="admin-app-shell"><aside className="admin-sidebar" aria-label="التنقل الإداري"><div className="admin-sidebar-brand"><Link href="/admin" aria-label="لوحة روافد"><span>ر</span><div><strong>منصة روافد</strong><small>لوحة الإدارة</small></div></Link></div><div className="admin-sidebar-user"><span>{profile.display_name||'مدير روافد'}</span><small>{profile.role==='owner'?'مالك المنصة':'مدير المنصة'}</small></div><nav className="admin-sidebar-nav">{groups.map((group)=><section key={group.label}><h2>{group.label}</h2>{group.items.map(([href,label])=><Link href={href} key={href}>{label}</Link>)}</section>)}</nav><div className="admin-sidebar-footer"><Link href="/">عرض الموقع</Link><Link href="/account">حسابي</Link></div></aside><div className="admin-mobile-bar"><Link className="admin-mobile-brand" href="/admin"><span>ر</span><strong>إدارة روافد</strong></Link><details className="admin-mobile-menu"><summary>القائمة</summary><div>{mobileItems.map(([href,label])=><Link href={href} key={href}>{label}</Link>)}<Link href="/">عرض الموقع</Link><Link href="/account">حسابي</Link></div></details></div><div className="admin-workspace">{children}</div></div>;
}
