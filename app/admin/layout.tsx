...
const groups: AdminNavGroup[] = [
  ...
  {
    label: 'الدليل والمجتمع',
    items: [
      ['/admin/specialists', 'المختصون والتوثيق'],
      ['/admin/centers', 'المراكز والفروع'],
      ['/admin/community', 'المتدربون والمتطوعون'],
  ...
  return <div className="admin-app-shell">
    <aside className="admin-sidebar" aria-label="التنقل الإداري">
      <div className="admin-sidebar-brand">
        <Link href="/admin" aria-label="لوحة روافد"><span>ر</span><div><strong>منصة روافد</strong><small>لوحة الإدارة</small></div></Link>
      </div>
      <div className="admin-sidebar-user"><span>{profile.display_name || 'مدير روافد'}</span><small>{profile.role === 'owner' ? 'مالك المنصة' : 'مدير المنصة'}</small></div>
      <nav className="admin-sidebar-nav">
        {groups.map((group) => <section key={group.label}><h2>{group.label}</h2>{group.items.map(([href,label]) => <Link href={href} key={href}>{label}</Link>)}</section>)}
      </nav>
      <div className="admin-sidebar-footer"><Link href="/">عرض الموقع</Link><Link href="/account">حسابي</Link></div>
    </aside>
