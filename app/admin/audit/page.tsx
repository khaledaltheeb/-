...
function compact(value:unknown){if(value==null)return'';try{const text=JSON.stringify(value);return text.length>900?`${text.slice(0,900)}…`:text}catch{return''}}
...
export default async function AuditPage({searchParams}:{searchParams:SearchParams}){
 ...
 {error&&<p className="system-message error">تعذر تحميل سجل العمليات.</p>}<div className="audit-list">{rows.map(row=><article className="audit-item" key={row.id}><div className="audit-head"><div><span className="status-badge">{row.entity_type}</span><strong>{row.action}</strong></div><time dateTime={row.created_at}>{date(row.created_at)}</time></div><div className="audit-meta"><span>المنفذ: {row.actor_name||row.actor_id||'النظام'}</span>{row.entity_id&&<span>الكيان: <bdi>{row.entity_id}</bdi></span>}</div>{row.before_data!=null&&<details><summary>قبل</summary><pre>{compact(row.before_data)}</pre></details>}{row.after_data!=null&&<details><summary>بعد</summary><pre>{compact(row.after_data)}</pre></details>}</article>)}{!error&&rows.length===0&&<p className="empty-state">لا توجد عمليات مطابقة.</p>}</div>
 </section></main>;
}
