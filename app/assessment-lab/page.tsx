...
export const metadata: Metadata = buildSeoMetadata({
  ...
  description: 'قطاع اختبر نفسك في روافد: 36 أداة متابعة ذاتية عربية غير تشخيصية بلا حفظ بيانات، مع فصل المقاييس النفسية المعروفة عن أدوات روافد وعدم نسخ أدوات محمية أو اختلاق درجات.',
  path: '/assessment-lab',
  index: true,
  follow: true,
  keywords: ['اختبر نفسك', 'اختبارات نفسية', 'فحص ذاتي', 'متابعة ذاتية', 'الصحة النفسية', 'النوم', 'القلق', 'الضغط النفسي', 'الاحتراق النفسي', 'PHQ-9', 'GAD-7', 'WHO-5', 'AUDIT'],
});
...
export default function AssessmentLabPage() {
  ...
  return <><SiteHeader/><main className={styles.page}>
    ...
    <section className={styles.hero}><div className={styles.shell}>
      ...
      <div className={styles.facts}><span><strong>36</strong> أداة متابعة محلية</span><span><strong>4</strong> أدوات مصدرية موثقة</span><span><strong>16</strong> بندًا لكل متابعة</span><span><strong>0</strong> إجابات محفوظة</span></div>
    </div></section>
    ...
    <section className={`${styles.shell} ${styles.method}`} aria-labelledby="method-title"><div className={styles.sectionHeading}><span className={styles.eyebrow}>المنهج</span><h2 id="method-title">نبني مكتبة فحص ذاتي مسؤولة، لا مكتبة «نتائج سريعة»</h2><p>الفحص الذاتي قد يكون مفيدًا عندما يساعد الشخص على تنظيم الملاحظة وطلب المساعدة المناسبة، لكنه يصبح مضللًا عندما يقلّد التشخيص أو يضع عتبات ودرجات بلا أساس. لذلك يعتمد هذا القطاع فصلًا واضحًا بين أدوات روافد المحلية والمقاييس المنشورة ذات المصادر والحقوق الخاصة بها.</p></div><div className={styles.methodGrid}>
      ...
      <article><h3>المقياس المعروف يبقى مقياسًا معروفًا</h3><p>PHQ-9 وGAD-7 وWHO-5 وAUDIT لا تُعاد تسميتها كأدوات روافد، ولا تُنسخ بنودها أو تُحسب درجاتها قبل تثبيت النسخة العربية وحقوق إعادة الاستخدام.</p></article>
    </div></section>
