...
with base as (
  select
    ...
    case
      when pg_catalog.jsonb_typeof(c.body_json->'blocks')='array' then pg_catalog.jsonb_array_length(c.body_json->'blocks')
      else 0
    end as block_count,
    coalesce((
      select pg_catalog.count(*)::integer
      from pg_catalog.jsonb_array_elements(
        case when pg_catalog.jsonb_typeof(c.body_json->'blocks')='array' then c.body_json->'blocks' else '[]'::jsonb end
      ) b
    ...
    (
      ...
      case
        when b.content_type='research' then
          ((b.reference_count>=2 and b.word_count>=300) or
           (b.reference_count=1 and b.word_count>=350 and b.block_count>=8 and b.heading_count>=3))
  ...
  select
    ...
    pg_catalog.count(*) filter(where s.status='published'::public.content_status and s.robots_index=true and s.published_at<=pg_catalog.now() and s.featured_image_url is null and coalesce(s.schema_json::text,'') !~ '"image"')::bigint as published_without_image_signal,
