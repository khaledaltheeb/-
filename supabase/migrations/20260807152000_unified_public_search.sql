...
as $$
  with input as (
    select
      ...
      greatest(1, least(coalesce(p_limit, 30), 50)) as lim
  ),
  content_results as (
    select
      ...
      coalesce(c.excerpt, left(coalesce(c.body_text, ''), 240)) as excerpt,
    ...
    where i.q <> ''
      ...
      and (
        ...
        or extensions.similarity(c.title, i.q) > 0.12
        or extensions.similarity(pg_catalog.array_to_string(c.search_aliases, ' '), i.q) > 0.12
  ...
  sector_results as (
    ...
    where i.q <> '' and s.is_active = true and s.visibility = 'public'
      and (extensions.similarity(s.name_ar, i.q) > 0.12 or s.name_ar ilike '%' || i.q || '%')
  ),
  category_results as (
    select
      'category'::text,
    ...
    where i.q <> '' and c.is_active = true and c.visibility = 'public'
      and (extensions.similarity(c.name_ar, i.q) > 0.12 or c.name_ar ilike '%' || i.q || '%')
  ),
  specialist_results as (
    select
      ...
      nullif(left(coalesce(s.bio, ''), 240), '') as excerpt,
      '/specialists/' || s.slug,
      greatest(
        extensions.similarity(s.full_name, i.q),
        coalesce(extensions.similarity(s.professional_title, i.q), 0),
        coalesce(extensions.similarity(pg_catalog.array_to_string(s.specialties, ' '), i.q), 0),
        case when s.full_name ilike '%' || i.q || '%' then 0.75 else 0 end
      )::double precision
    from public.specialists s cross join input i
    where i.q <> '' and s.is_active = true and s.verification = 'verified'::public.verification_status
      and (
        extensions.similarity(s.full_name, i.q) > 0.12
        or extensions.similarity(coalesce(s.professional_title, ''), i.q) > 0.12
        or extensions.similarity(pg_catalog.array_to_string(s.specialties, ' '), i.q) > 0.12
        or s.full_name ilike '%' || i.q || '%'
        or coalesce(s.professional_title, '') ilike '%' || i.q || '%'
      )
  ),
  center_results as (
    select
      ...
      c.slug,
      c.name,
      nullif(pg_catalog.concat_ws('، ', c.city, c.country), '') as subtitle,
      nullif(left(coalesce(c.description, ''), 240), '') as excerpt,
      '/centers/' || c.slug,
      greatest(
        extensions.similarity(c.name, i.q),
        coalesce(extensions.similarity(c.city, i.q), 0),
        case when c.name ilike '%' || i.q || '%' then 0.75 else 0 end
      )::double precision
    from public.centers c cross join input i
    where i.q <> '' and c.is_active = true and c.verification = 'verified'::public.verification_status
      and (
        extensions.similarity(c.name, i.q) > 0.12
        or extensions.similarity(coalesce(c.city, ''), i.q) > 0.12
        or c.name ilike '%' || i.q || '%'
      )
  ),
  combined as (
