-- Data minimization: routine driving-risk alerts do not disclose live location.
-- Location is included only for explicit/authorized incident-help escalation.
...
begin
  ...
  if v_event='risky_driving' then
    if (select count(*) from public.circle_messages m where m.sender_id=v_uid and m.template_key='safe_drive_risk' and m.created_at>now()-interval '1 hour') >= 30 then
      raise exception 'drive risk alert rate limit exceeded';
    end if;
    ...
    if (select count(*) from public.circle_messages m where m.sender_id=v_uid and m.template_key='safe_drive_incident' and m.created_at>now()-interval '1 hour') >= 12 then
      raise exception 'drive incident alert rate limit exceeded';
    end if;
    ...
    insert into public.notifications(user_id,kind,title,body,data)
      values(
        ...
        jsonb_build_object(
          ...
          'location_shared',v_event<>'risky_driving'
        )
      );
    v_count := v_count + 1;
  end loop;
