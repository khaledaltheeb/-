...
begin
  ...
  set schema_json=(coalesce(c.schema_json,'{}'::jsonb)-'release_blocker'-'public_route_verification') || pg_catalog.jsonb_build_object(
    ...
    'public_route_verification',pg_catalog.jsonb_build_object(
      ...
      'robots_index_match',true,'release_token_match',true,'sitemap_present',true,'sitemap_http_status',v_sitemap_status,
      'canonical_url',c.canonical_url,'release_token',v_token,'verification_mode','two-phase-live-route',
      'verifier','database-live-http-v2','public_origin',pg_catalog.rtrim(p_base_url,'/'),
  ...
  values(null,'content',p_id::text,'pediatric_oncology_release_verified',pg_catalog.jsonb_build_object(
    'http_status',v_page_status,'sitemap_http_status',v_sitemap_status,'release_token',v_token,
    'verifier','database-live-http-v2','public_origin',pg_catalog.rtrim(p_base_url,'/')));

  return pg_catalog.jsonb_build_object(
    'id',p_id,'status','published','verification','passed','http_status',v_page_status,
    'canonical_match',true,'robots_index_match',true,'release_token_match',true,'sitemap_present',true
  );
end;
$function$;
