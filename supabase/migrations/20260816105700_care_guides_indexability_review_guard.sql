...
drop trigger if exists zzzz_care_guides_indexability_review_guard on public.content;
create trigger zzzz_care_guides_indexability_review_guard
before insert or update of status, robots_index, schema_json, reviewer_display_name, reviewer_credentials, last_reviewed_at, canonical_url, body_text, references_json, medical_disclaimer
on public.content
for each row
execute function private.care_guides_indexability_review_guard();
