...
begin
  ...
  -- V8+ state invariant: an indexable Care Guide must be a published,
  -- publication-ready record. Older V7 records remain grandfathered and
  -- are not retroactively forced into the V8 metadata contract.
  ...
  if tg_op = 'UPDATE' then
    ...
    v_review_metadata_change := v_is_wave004 and (coalesce(new.robots_index, false) or v_ready);
    v_wave_revalidation := v_is_wave004 and (coalesce(new.robots_index, false) or v_ready);
  end if;

  -- Preserve the V8 central-disclaimer contract whenever a V8+ Care Guide
  -- is activated or its release schema/disclaimer state changes.
  ...
  -- Wave 004 deliberately has a stricter release policy than earlier Care
  -- Guide waves: genuine independent human review is required before this
  -- batch can become ready/indexable. Do not infer that policy retroactively
  -- for Wave 001-003 or V7 records.
  if v_is_wave004 and (v_activation or v_review_metadata_change or v_wave_revalidation) then
    if nullif(pg_catalog.btrim(coalesce(new.reviewer_display_name, '')), '') is null then
      raise exception 'Wave 004 care guide indexability/readiness requires a recorded human reviewer';
    end if;
    if nullif(pg_catalog.btrim(coalesce(new.reviewer_credentials, '')), '') is null then
      raise exception 'Wave 004 care guide indexability/readiness requires reviewer credentials';
    end if;
    if new.last_reviewed_at is null then
      raise exception 'Wave 004 care guide indexability/readiness requires a recorded review date';
    end if;
    if new.last_reviewed_at > pg_catalog.now() then
      raise exception 'Wave 004 care guide review date cannot be in the future';
    end if;
    if nullif(pg_catalog.btrim(coalesce(new.author_display_name, '')), '') is not null
      ...
      raise exception 'Wave 004 care guide indexability/readiness requires an independent reviewer distinct from the visible author';
    end if;
