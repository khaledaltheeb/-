...
begin
  ...
  select pg_catalog.count(*)::integer
    into v_ar_words
  from pg_catalog.regexp_split_to_table(coalesce(v_body,''),'[[:space:]]+') as token
  where token ~ '[ء-ي]';
  ...
  if v_ar_words < 2500 then raise exception 'gold guide requires at least 2500 Arabic words using the exact V6 counter; found %',v_ar_words; end if;
