...
function protocolSteps(value: unknown) {
  ...
  return blocks.flatMap((block, index) => {
    ...
    const items = next?.type === 'list' && Array.isArray(next.items) ? next.items.filter((item): item is string => typeof item === 'string') : [];
...
export default function CapabilityArticlePage({ record, routeSlug, registryItems = [] }: Props) {
  ...
  const contentSchema: Record<string, unknown> = role === 'registry'
    ? { ...common, '@type': 'CollectionPage', mainEntity: { '@type': 'ItemList', numberOfItems: registry.length, itemListElement: registry.map((item) => ({ '@type': 'ListItem', position: item.rank, name: item.title, url: item.href.startsWith('https://') ? item.href : `${SITE_URL}${item.href}` })) } }
    : role === 'hub'
      ? { ...common, '@type': 'CollectionPage', hasPart: [
      ...
      : role === 'protocol' && steps.length > 0
        ? { ...common, '@type': 'HowTo', step: steps.map((step) => ({ '@type': 'HowToStep', position: step.position, name: step.name, text: step.text })) }
    ...
    <>
      ...
      <main className="article-shell">
        ...
        <nav className="breadcrumbs" aria-label="مسار الصفحة">
          <Link href="/">الرئيسية</Link><span>/</span><Link href="/sectors">القطاعات</Link><span>/</span><Link href="/sectors/capabilities">لنرتقي بقدراتهم</Link>
          {routeSlug ? <><span>/</span><Link href="/capabilities/">مرجع القدرات</Link></> : null}
