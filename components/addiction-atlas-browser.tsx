...
import { countAdfDrugFactReferences, getAdfDrugFactReference } from '@/lib/adf-addiction';
...
export default function AddictionAtlasBrowser({ substances, methodology, comparisons }: Props) {
  ...
  const [adfCoverage, setAdfCoverage] = useState('all');
  ...
  const adfMappedCount = useMemo(() => countAdfDrugFactReferences(substances), [substances]);
  const filtered = useMemo(() => {
    ...
    const rows = substances.filter((item) => {
      ...
      const hasAdf = Boolean(getAdfDrugFactReference(item));
      if (adfCoverage === 'mapped' && !hasAdf) return false;
      if (adfCoverage === 'unmapped' && hasAdf) return false;
  ...
  }, [substances, query, category, evidence, adfCoverage, sortKey, sortDir]);
  ...
  return <>
    <div className={styles.controls}>
      ...
      <label><span>مرجع ADF الموازٍ</span><select value={adfCoverage} onChange={(event) => setAdfCoverage(event.target.value)}><option value="all">كل السجلات</option><option value="mapped">له Drug Facts مباشر</option><option value="unmapped">بلا مطابقة مباشرة</option></select></label>
    </div>
    <p className={styles.count}><strong>{filtered.length}</strong> مادة/عائلة ظاهرة. توجد إحالة ADF مباشرة لـ <strong>{adfMappedCount}</strong> من أصل {substances.length}. عدم وجود إحالة لا يعني نقصًا في الدليل أو أمان المادة.</p>
    ...
    <section className={styles.section} aria-labelledby="atlas-compare-title">
      ...
      <p>المقارنة تعرض محاور مستقلة ولا تعني أن المادة ذات الدرجة الأقل في محور معين آمنة. للمقارنة السريرية الموسعة افتح مركز المقارنات.</p>
      ...
      {a && b && a.slug !== b.slug ? <>
        <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>المحور</th><th>{a.display_name_ar}</th><th>{b.display_name_ar}</th></tr></thead><tbody><tr><th>الفئة</th><td>{a.class_ar}</td><td>{b.class_ar}</td></tr>{RISK_KEYS.map((key) => <tr key={key}><th>{methodology.risk_dimensions[key].label_ar}</th><td>{riskBadge(a.risk[key])}</td><td>{riskBadge(b.risk[key])}</td></tr>)}<tr><th>قوة الدليل</th><td>{a.evidence_grade}</td><td>{b.evidence_grade}</td></tr><tr><th>ADF Drug Facts</th><td>{getAdfDrugFactReference(a) ? <a href={getAdfDrugFactReference(a)!.url} target="_blank" rel="noopener noreferrer">مرجع خارجي</a> : 'لا توجد مطابقة مباشرة'}</td><td>{getAdfDrugFactReference(b) ? <a href={getAdfDrugFactReference(b)!.url} target="_blank" rel="noopener noreferrer">مرجع خارجي</a> : 'لا توجد مطابقة مباشرة'}</td></tr></tbody></table></div>
        <div className={styles.actions}><Link href={`/addiction/substances/${a.slug}/`}>صفحة {a.display_name_ar}</Link><Link href={`/addiction/substances/${b.slug}/`}>صفحة {b.display_name_ar}</Link><Link href="/addiction/compare/">المقارنة السريرية الموسعة</Link>{canonicalComparison ? <Link href={`/addiction/compare/${canonicalComparison.slug}/`}>المقارنة التحريرية الكاملة</Link> : null}</div>
