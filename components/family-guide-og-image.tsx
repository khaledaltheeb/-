...
export function familyGuideOgImage(title: string) {
  ...
  return new ImageResponse((
    <div dir="rtl" style={{ width:'100%',height:'100%',display:'flex',position:'relative',overflow:'hidden',background:'linear-gradient(135deg,#f5fffb 0%,#ffffff 48%,#fff6e9 100%)',color:'#143b42',fontFamily:'sans-serif' }}>
      ...
      <div style={{ width:'100%',padding:'76px 82px',display:'flex',flexDirection:'column',justifyContent:'space-between',zIndex:2 }}>
        <div style={{ display:'flex',flexDirection:'column',alignItems:'flex-start',width:820 }}>
          <div style={{ display:'flex',padding:'10px 18px',borderRadius:999,background:'#e5f6f1',color:'#075e5d',fontSize:23,fontWeight:800 }}>دليل الأسرة · منصة روافد</div>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:16 }}><div style={{ width:54,height:54,borderRadius:18,display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#0f8f88,#3ec7ad)',color:'#fff' }}><RawafidMark accent="#f4b942" style={{width:38,height:38}} /></div><div style={{ display:'flex',flexDirection:'column' }}><div style={{ display:'flex',fontSize:25,fontWeight:900 }}>منصة روافد</div><div style={{ display:'flex',fontSize:17,color:'#657d82' }}>العافية النفسية · الدمج · التمكين</div></div></div>
      </div>
    </div>
  ), FAMILY_GUIDE_OG_SIZE);
}
