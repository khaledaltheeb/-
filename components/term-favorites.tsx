...
export default function TermFavorites({ terms }: { terms: TerminologyToolTerm[] }) {
  ...
  useEffect(()=>{let active=true;const id=window.setTimeout(()=>{if(!active)return;try{const value=JSON.parse(localStorage.getItem(KEY)||'[]');if(Array.isArray(value))setFavorites(value.filter((x):x is string=>typeof x==='string'&&bySlug.has(x)).slice(0,100))}catch{}},0);return()=>{active=false;window.clearTimeout(id)}},[bySlug]);
