...
export default async function SiteHeader() {
  ...
  return <>
    <header className="site-header"><div className="site-header-inner">
      <a className="brand" href="/" aria-label="منصة روافد - الرئيسية"><span className="brand-mark" aria-hidden="true"><svg viewBox="0 0 48 48" focusable="false"><circle className="logo-source" cx="13" cy="12" r="3.2"/><path className="logo-stream" d="M13 16c1 8 5.5 10.5 12 12.5S35.5 34 36 40"/><path className="logo-stream" d="M6.5 22c7.5 0 10.5 4 18.5 6.5S35.5 34 36 40"/><path className="logo-stream" d="M21 8c-2 8 0 14 4 20.5S34.5 35 36 40"/><path className="logo-stream" d="M31 15c-3.5 4-5.5 8-6 13.5"/></svg></span><span className="brand-copy"><strong>منصة روافد</strong><small>معرفة تقود إلى أثر</small></span></a>
      <nav className="desktop-nav" aria-label="التنقل الرئيسي"><a href="/">الرئيسية</a><details className="nav-dropdown mega-nav"><summary>استكشف روافد</summary><div className="nav-dropdown-panel mega-nav-panel"><div className="nav-dropdown-heading"><div><strong>الوصول إلى منصة روافد حسب احتياجك</strong><span>قطاعات معرفية وخدمية، مسارات بحث، ودليل خدمات في مكان واحد</span></div><a href="/search">فتح البحث المتقدم ←</a></div><div className="mega-nav-layout"><section className="mega-nav-column mega-nav-sectors"><h2>القطاعات</h2><div className="mega-sector-grid">{sectors.map((sector)=><a key={sector.slug} href={`/sectors/${sector.slug}`}><i style={{background:sector.accent||'#08716d'}} aria-hidden="true"/><span>{sector.name_ar}</span></a>)}{sectors.length===0&&<div className="mega-empty"><strong>لا توجد قطاعات عامة متاحة حاليًا</strong><span>ستظهر القطاعات هنا بعد اعتمادها للنشر العام.</span></div>}</div></section><section className="mega-nav-column"><h2>ابدأ من احتياجك</h2><div className="mega-intent-list">{intentLinks.map((link)=><a href={link.href} key={link.href}><strong>{link.label}</strong><span>{link.detail}</span></a>)}</div></section><section className="mega-nav-column mega-services"><h2>الدليل والخدمات</h2><div>{serviceLinks.map((link)=><a href={link.href} key={link.href}><PlatformIcon name={link.icon} size={19}/><span>{link.label}</span></a>)}</div>{signedIn&&<div className="mega-member-links"><a href="/messages">الرسائل</a><a href="/appointments">المواعيد</a><a href="/notifications">الإشعارات</a></div>}</section></div></div></details>{primaryLinks.map((link)=><a key={link.href} href={link.href}>{link.label}</a>)}{signedIn&&<a href="/messages">الرسائل</a>}</nav>
