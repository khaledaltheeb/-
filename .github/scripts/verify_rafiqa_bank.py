...
def read_encoded(category: str) -> bytes:
    for count in PART_COUNTS:
        parts = [ASSET_DIR / f"{category}.part{i:02d}.b64" for i in range(count)]
        if all(p.is_file() for p in parts):
            return b"".join(p.read_bytes().strip() for p in parts)
    layouts = " or ".join(str(n) for n in PART_COUNTS)
    raise AssertionError(f"{category}: missing complete {layouts}-part asset layout")
...
def main() -> None:
    ...
    for category in CATEGORIES:
        ...
        if len(lines) != EXPECTED_PER_CATEGORY:
            raise AssertionError(f"{category}: expected {EXPECTED_PER_CATEGORY} messages, found {len(lines)}")
        for index, line in enumerate(lines, start=1):
            fields = line.split("\t")
            if len(fields) < 20:
                raise AssertionError(f"{category}:{index}: expected >=20 TSV fields, found {len(fields)}")
            ...
            if category_code != category:
                raise AssertionError(f"{category}:{index}: category_code={category_code!r} does not match asset")
    ...
    if len(global_ids) != EXPECTED_TOTAL:
        raise AssertionError(f"expected {EXPECTED_TOTAL} globally unique IDs, found {len(global_ids)}")
