 ...
 * most once in the returned forest, so public navigation remains complete
 * even while taxonomy edits are happening concurrently.
 */
export function buildPublicCategoryForest<T extends PublicCategoryTreeItem>(categories: T[]): PublicCategoryTreeNode<T>[] {
  ...
  // A pure cycle has no declared root. Promote the first still-unvisited node
  // and continue until every public category is reachable from the UI.
  for (const category of ordered) {
