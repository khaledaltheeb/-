...
export async function getAddictionItems(): Promise<AddictionItem[]> {
  const supabase = await createClient();
  const { data } = await supabase.from('content').select('slug,title,canonical_url,schema_json,published_at')
    .like('slug', 'addiction-%').eq('status', 'published').lte('published_at', new Date().toISOString()).limit(200);
  const rows = Array.isArray(data) ? data : [];
  return rows.flatMap((row) => {
    const schema = asRecord(row.schema_json);
    if (asString(schema?.page_role) !== 'addiction_condition') return [];
    const rank = Number(schema?.addiction_rank);
    if (!Number.isInteger(rank) || rank < 1 || rank > 100) return [];
    const slug = asString(row.slug).replace(/^addiction-/, '');
    return [{ rank, slug, title: asString(row.title), titleEn: asString(schema?.title_en), category: asString(schema?.addiction_category) || 'اضطراب استخدام أو سلوك إدماني', href: asString(row.canonical_url) || `/addiction/${slug}/` }];
