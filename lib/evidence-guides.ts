...
const FIELDS='id,slug,title,excerpt,body_json,body_text,seo_title,seo_description,canonical_url,robots_index,robots_follow,published_at,updated_at,primary_keyword,secondary_keywords,semantic_terms,author_display_name,last_reviewed_at,references_json,medical_disclaimer,schema_json';
...
export async function getEvidenceGuideItems():Promise<EvidenceGuideRecord[]>{
 const supabase=await createClient();
 const {data,error}=await supabase.from('content').select(FIELDS).eq('content_type','guide').eq('status','published').like('canonical_url','/evidence-guides/%').order('title',{ascending:true}).limit(500);
 if(error) throw error; return ((data??[]) as unknown as EvidenceGuideRecord[]).filter((r)=>isPublishedNow(r.published_at));
}
export async function getEvidenceGuideHub():Promise<EvidenceGuideRecord|null>{
 const supabase=await createClient(); const {data,error}=await supabase.from('content').select(FIELDS).eq('slug','evidence-guides-hub').eq('status','published').maybeSingle();
...
export async function getEvidenceGuideRecord(routeSlug:string):Promise<EvidenceGuideRecord|null>{
 ...
 const supabase=await createClient(); const {data,error}=await supabase.from('content').select(FIELDS).eq('content_type','guide').eq('status','published').eq('canonical_url',`/evidence-guides/${safe}/`).maybeSingle();
 if(error) throw error; const r=data as unknown as EvidenceGuideRecord|null; return r&&isPublishedNow(r.published_at)?r:null;
}
