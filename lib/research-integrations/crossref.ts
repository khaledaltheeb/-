...
function firstText(value: unknown) {
  return asArray(value).map(text).find(Boolean) || text(value);
}
...
function authors(value: unknown): EvidenceAuthor[] {
  return asArray(value).flatMap((item) => {
    ...
    if (!author) return [];
    const displayName = text(author.name) || [text(author.given), text(author.family)].filter(Boolean).join(' ');
    if (!displayName) return [];
    const affiliations = asArray(author.affiliation).flatMap((affiliation) => {
      const row = record(affiliation);
      const name = text(row?.name);
      return name ? [{ name, original: name, ror_id: null, country_code: null }] : [];
    });
...
function dateParts(value: unknown) {
  ...
  if (!Number.isInteger(year) || year < 1000 || year > 3000) return { date: null, year: null };
...
function publicationDate(row: JsonRecord) {
  for (const key of ['published-online', 'published-print', 'published', 'issued', 'created']) {
    const parsed = dateParts(row[key]);
...
function relations(row: JsonRecord): EvidenceRelation[] {
  ...
  if (relation) {
    for (const [relationType, values] of Object.entries(relation)) {
      for (const value of asArray(values)) {
        ...
        const identifier = text(item?.id);
        if (!identifier) continue;
        output.push({
          relation_type: relationType,
          identifier,
          identifier_type: text(item?.['id-type']),
          source: text(item?.['asserted-by']) || 'crossref',
        });
      }
    }
  }
  for (const value of asArray(row['update-to'])) {
    const item = record(value);
    const identifier = text(item?.DOI) || text(item?.doi);
    if (!identifier) continue;
    output.push({
      relation_type: text(item?.type) || 'update-to',
      identifier,
      identifier_type: 'doi',
      source: text(item?.source) || 'crossref',
    });
  }
  return output;
}

function explicitlyRetracted(row: JsonRecord, normalizedRelations: EvidenceRelation[]) {
  if (normalizedRelations.some((relation) => relation.relation_type.toLowerCase().includes('retract'))) return true;
  return asArray(row['update-to']).some((value) => text(record(value)?.type)?.toLowerCase() === 'retraction');
}
...
function normalizeCrossrefResult(value: unknown, retrievedAt: string, query: string, version: string | null): EvidenceRecord | null {
  ...
  if (!row) return null;
  const doi = text(row.DOI)?.toLowerCase() || null;
  const url = text(row.URL) || (doi ? `https://doi.org/${doi}` : null);
  if (!doi && !url) return null;
  const publication = publicationDate(row);
  const normalizedRelations = relations(row);
  return {
    provider: 'crossref',
    provider_id: doi || url as string,
    title: firstText(row.title),
    abstract: stripMarkup(row.abstract),
    publication_type: text(row.type),
    publication_year: publication.year,
    publication_date: publication.date,
    journal: firstText(row['container-title']),
    publisher: text(row.publisher),
    authors: authors(row.author),
    identifiers: { doi: doi || undefined },
    relations: normalizedRelations,
    cited_by_count: numberValue(row['is-referenced-by-count']),
    is_open_access: null,
    is_retracted: explicitlyRetracted(row, normalizedRelations),
    url,
    provenance: {
      retrieved_at: retrievedAt,
      endpoint: CROSSREF_BASE,
...
function normalizeFilterDate(value: string | null | undefined) {
  ...
  if (Number.isNaN(parsed)) throw new TypeError('Crossref filter dates must be valid ISO dates.');
  return new Date(parsed).toISOString();
}

export async function searchCrossref(options: CrossrefSearchOptions): Promise<EvidenceSearchPage> {
  const query = options.query.trim();
  if (query.length < 2 || query.length > 1_000) throw new TypeError('Crossref query must contain 2-1000 characters.');
  const rows = Math.max(1, Math.min(100, options.rows ?? 25));
  const mailto = options.mailto?.trim() || 'contact@healthrenewal.org';
  ...
  const fromUpdate = normalizeFilterDate(options.from_update_date);
  const fromIndex = normalizeFilterDate(options.from_index_date);
  if (fromUpdate) filters.push(`from-update-date:${fromUpdate}`);
  if (fromIndex) filters.push(`from-index-date:${fromIndex}`);
  ...
  const response = await requestJson<JsonRecord>({
    ...
    init: {
      headers: {
        ...
        'User-Agent': `Rawafid/1.1 (+https://healthrenewal.org; mailto:${mailto})`,
      },
    },
  });
  const message = record(response.message);
  const retrievedAt = new Date().toISOString();
  const version = text(response['message-version']);
  return {
    records: asArray(message?.items).map((item) => normalizeCrossrefResult(item, retrievedAt, query, version)).filter((item): item is EvidenceRecord => Boolean(item)),
    total: numberValue(message?.['total-results']),
    next_cursor: text(message?.['next-cursor']),
    provider_version: version,
  };
}
