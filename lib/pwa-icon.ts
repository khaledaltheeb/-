...
export function createPwaIcon(size: number) {
  return new ImageResponse(
    React.createElement(
      ...
      React.createElement('div', { style: { display:'flex', alignItems:'center', justifyContent:'center', width:size*.62, height:size*.62, borderRadius:size*.18, border:`${Math.max(2,Math.round(size*.012))}px solid rgba(255,255,255,.2)`, background:'rgba(255,255,255,.08)', fontSize:size*.34, lineHeight:1, fontWeight:900, letterSpacing:-size*.02 } }, 'R'),
    ),
    {
      width: size,
      height: size,
