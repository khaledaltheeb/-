...
const textOptions = (values) => unique(values).map((value) => option(value));
...
function planningTask(random, level, mode) {
  ...
  const items=['أ','ب','ج','د']; const answer='أ → ب → ج → د'; return {prompt:`رتّب العناصر إذا كان أ قبل ب، وب قبل ج، وج قبل د.`,answer,options:[answer,'ب → أ → ج → د','أ → ج → ب → د','د → ج → ب → أ'],rationale:'الترتيب الوحيد الذي يحقق القيود الثلاثة هو أ ثم ب ثم ج ثم د.',difficultyDescriptor:`${Math.min(3,1+level)} قيود ترتيب صريحة`,difficultySignature:`constraint-planning:${level}`};
}
