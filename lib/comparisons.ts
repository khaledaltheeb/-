...
export async function getComparisonRecord(routeSlug?: string): Promise<ComparisonRecord | null> {
  ...
  const preferredSlug = comparisonContentSlug(routeSlug);
  const candidates = routeSlug ? [preferredSlug, routeSlug] : [preferredSlug];
  ...
  const rows = (Array.isArray(data) ? data : []) as ComparisonRecord[];
  const preferred = rows.find((row) => row.slug === preferredSlug);
  if (preferred) return preferred;
  const direct = routeSlug ? rows.find((row) => row.slug === routeSlug && row.content_type === 'comparison') : null;
  return direct ?? null;
}
