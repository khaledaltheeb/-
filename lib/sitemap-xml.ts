...
export function sitemapResponse(entries: SitemapEntry[]) {
  ...
  const urls = safeEntries.map((entry) => {
    ...
    return `<url><loc>${escapeXml(absolute(entry.path))}</loc>${lastmod ? `<lastmod>${lastmod}</lastmod>` : ''}${entry.changeFrequency ? `<changefreq>${escapeXml(entry.changeFrequency)}</changefreq>` : ''}${priority !== undefined ? `<priority>${priority.toFixed(1)}</priority>` : ''}</url>`;
  }).join('');
