...
import { effectiveSitemapPriority } from '@/lib/sitemap-priority';
...
function formatPriority(value: number) { return value.toFixed(2).replace(/\.?0+$/, ''); }
export function sitemapResponse(entries: SitemapEntry[]) {
  const safeEntries = INDEXING_ENABLED ? entries : [];
  const urls = safeEntries.map((entry) => {
    const lastmod = entry.lastModified ? new Date(entry.lastModified).toISOString() : undefined;
    const priority = effectiveSitemapPriority(entry.path, entry.priority);
    return `<url><loc>${escapeXml(absolute(entry.path))}</loc>${lastmod ? `<lastmod>${lastmod}</lastmod>` : ''}${entry.changeFrequency ? `<changefreq>${escapeXml(entry.changeFrequency)}</changefreq>` : ''}${priority !== undefined ? `<priority>${formatPriority(priority)}</priority>` : ''}</url>`;
  }).join('');
...
export function sitemapIndexResponse(paths:string[]){
 const safe=INDEXING_ENABLED?paths:[];
