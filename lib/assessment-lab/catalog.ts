...
const questionTextCorrections: Record<string, Record<string, string>> = {
  ...
  'problem-solving': {
    'كم بديلًا عمليًا استطعت التفكير فيه قبل الاختيار؟': 'كم مرة فكرت في أكثر من بديل عملي قبل الاختيار؟',
  },
  'health-worry': {
    'هل استطعت التفكير في تفسير شائع وغير خطير للإحساس؟': 'هل استطعت تذكر أن الإحساس قد تكون له تفسيرات متعددة بدل الجزم فورًا بأخطر تفسير؟',
    'هل استطعت تأجيل فحص غير عاجل عندما لاحظت أنه لتهدئة القلق فقط؟': 'إذا لم توجد علامة إنذار أو توصية طبية بالمتابعة، هل استطعت تأجيل تحقق متكرر لاحظت أنه لتهدئة القلق فقط؟',
  },
  'recovery-safety': {
    'إذا كان الخطر فوريًا الآن، هل ستتوقف عن الأداة وتطلب مساعدة مباشرة؟': 'هل يوجد خطر فوري الآن يدفعك إلى العودة للسلوك الضار الذي تحاول التعافي منه؟',
  },
};
...
const questionSafetyOverrides: Record<string, Record<string, AssessmentSafetySignal>> = {
  ...
  'boundary-setting': {
    'هل أدى وضع الحد إلى تهديد أو ترهيب؟': {
      triggerValues: ['نعم'],
      level: 'urgent',
      kind: 'personal-safety',
      title: 'التهديد أو الترهيب يغيّر الأولوية من وضع الحدود إلى الأمان',
      message: 'إذا أدى وضع الحد إلى تهديد أو ترهيب، لا تستخدم هذه المتابعة لتشجيع مواجهة مباشرة. ركز على الأمان، واستخدم وسيلة تواصل آمنة إن أمكن، واطلب دعمًا مباشرًا من شخص موثوق أو جهة حماية أو خدمات الطوارئ المحلية بحسب شدة الخطر.',
    },
  },
  'recovery-safety': {
    'إذا كان الخطر فوريًا الآن، هل ستتوقف عن الأداة وتطلب مساعدة مباشرة؟': {
      triggerValues: ['نعم'],
      level: 'urgent',
      kind: 'recovery-support',
      title: 'الخطر الفوري يحتاج إلى مساعدة مباشرة الآن',
      message: 'إذا كان خطر العودة إلى السلوك الضار فوريًا الآن، أوقف المتابعة وابتعد قدر الإمكان عن الشيء أو الموقف الذي يزيد الخطر. تواصل مباشرة مع شخص داعم أو جهة مهنية أو خدمات الطوارئ المحلية بحسب طبيعة الخطر، ولا تعتمد على هذه الأداة لتقدير مستوى الخطر أو التعامل معه وحدك.',
    },
  },
};
...
const sourceStatusLabels: Record<string, string> = {
  ...
  'official-arabic-unrestricted-with-attribution': 'نسخة عربية منشورة من المصدر ومسموح باستخدام الترجمات دون تقييد مع الالتزام بإقرار حقوق النشر والاستشهاد المطلوب — تُطابق النسخة والحساب قبل التفعيل التفاعلي',
