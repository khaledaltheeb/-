...
if (!discoveryStep.includes('continue-on-error: true')) fail('Rich Discovery advisory semantics must remain unchanged');
...
for (const token of ['node scripts/seo-gate.mjs','node scripts/rich-discovery-gate.mjs','node scripts/content-richness-gate.mjs',"grep -Eq 'cacheHits=[1-9][0-9]*' /tmp/rawafid-rich-discovery.log","grep -Eq 'cacheHits=[1-9][0-9]*' /tmp/rawafid-content-richness.log"]) {
  if (!integration.includes(token)) fail(`full integration must prove shared-cache use: missing ${token}`);
}

if (!process.exitCode) console.log('SEO gate reliability contract passed: one audited HTML representation is reused without reducing coverage or blocking semantics.');
