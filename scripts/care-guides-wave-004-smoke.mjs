...
async function verifyReviewedPublishedRoute(route, titleMarker) {
  try {
    ...
    if (!robots.includes('index') || !robots.includes('follow') || robots.includes('noindex') || robots.includes('nofollow')) {
      console.error(`WAVE004 ${route}: reviewed page must be index,follow without noindex/nofollow; got ${robots || 'missing'}`);
      failed = true;
    }
...
console.log(`Wave 004 rendered smoke passed for ${reviewedPublished.length} reviewed indexable pages.`);
