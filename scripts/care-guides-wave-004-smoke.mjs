...
// Wave 004 has no publication authorization or completed human review yet.
// All currently materialized routes must therefore stay directly accessible
// but non-indexable until an explicit per-page release decision is recorded.
const heldPublished = [
...
async function verifyHeldRoute(route, titleMarker) {
  try {
    ...
    console.log(`WAVE004 ${route}: 200 + noindex,follow + canonical + disclaimer + references checked`);
  } catch (error) {
...
for (const [route, titleMarker] of heldPublished) {
  await verifyHeldRoute(route, titleMarker);
}

if (failed) process.exit(1);
console.log(`Wave 004 rendered smoke passed for ${heldPublished.length} held materialized pages.`);
