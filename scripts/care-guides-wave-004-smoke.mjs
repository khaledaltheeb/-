...
// These routes are currently published as canonical public pages. The historical
// 2026-08-16 evidence audit remains an accurate record of the earlier pre-indexing
// checkpoint; this runtime contract protects the platform's later publication state
// without rewriting that historical audit or claiming a human review it did not record.
const indexablePublished = [
...
async function verifyPublishedRoute(route, titleMarker) {
  try {
    ...
    if (!robots.includes('index') || !robots.includes('follow') || robots.includes('noindex') || robots.includes('nofollow')) {
      console.error(`WAVE004 ${route}: expected published canonical robots index,follow without noindex/nofollow; got ${robots || 'missing'}`);
      failed = true;
    }
...
for (const [route, titleMarker] of indexablePublished) {
  await verifyPublishedRoute(route, titleMarker);
}

if (failed) process.exit(1);
console.log(`Wave 004 rendered smoke passed for ${indexablePublished.length} published indexable canonical pages; historical pre-indexing audit remains unchanged.`);
