...
const publicRoutes=['/','/theme-preview','/search','/specialists','/centers','/community','/about','/offline','/share','/manifest.webmanifest','/sw.js','/robots.txt','/pwa-icon-180','/pwa-icon-192','/pwa-icon-512'];
const protectedRoutes=['/messages','/appointments','/notifications','/account','/admin','/specialist/content'];
let failed=false;
for(const route of publicRoutes){try{const response=await fetch(`${base}${route}`,{redirect:'manual'});if(response.status<200||response.status>=400){console.error(`PUBLIC ${route}: ${response.status}`);failed=true;}else if(route.startsWith('/pwa-icon-')&&!String(response.headers.get('content-type')||'').includes('image/png')){console.error(`PUBLIC ${route}: expected image/png`);failed=true;}else console.log(`PUBLIC ${route}: ${response.status}`);}catch(error){console.error(`PUBLIC ${route}:`,error);failed=true;}}
