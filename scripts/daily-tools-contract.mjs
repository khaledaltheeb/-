...
if (hub && hubToolLinks(hub).size !== EXPECTED_TOOLS) fail(`Daily Tools hub must expose exactly ${EXPECTED_TOOLS} unique tool routes`);

let runtimeMatches = 0;
for (const [sourcePath, record] of standard) {
  const { steps, fields } = runtimeShape(record);
  if (steps.length !== 4) { fail(`${sourcePath}: expected four source-derived steps, found ${steps.length}`); continue; }
  if (fields.length < 3) { fail(`${sourcePath}: expected at least three source-derived fields, found ${fields.length}`); continue; }
  if (!/(?:localStorage|المتصفح|محلي)/u.test(text(record?.body_text))) { fail(`${sourcePath}: local/privacy marker missing`); continue; }
  runtimeMatches += 1;
}
...
const files = {
  ...
  generator: 'scripts/build-daily-tools-route-manifest.mjs',
...
for (const [name, file] of Object.entries(files)) if (!fs.existsSync(file)) fail(`${name} missing: ${file}`);
...
if (!bad) {
  ...
  const hubSource = read(files.hub);
  ...
  for (const marker of ['deriveDailyToolSpec', 'تشمل حقول المتابعة:', 'fieldKind']) if (!parser.includes(marker)) fail(`parser marker missing: ${marker}`);
  for (const marker of ["import 'server-only'", 'getCloudflareContext', "{ async: true }", "@/generated/daily-tools-routes.json", "ASSET_ROOT = '/daily-tools-data/records'", 'readCloudflareAsset', 'getDailyToolPage', 'getDailyToolRoutes', 'dailyToolMetadata', 'index: true']) {
    if (!catalog.includes(marker)) fail(`Cloudflare asset catalog marker missing: ${marker}`);
  }
  if (/readFileSync|loadPayload\s*\(/.test(catalog)) fail('runtime catalog must not synchronously read the multi-megabyte source corpus');
  if (/createClient|getLegacyPreservedPage|get_legacy_preserved_page/.test(catalog)) fail('Daily Tools catalog must not depend on Supabase legacy RPC at runtime');

  for (const marker of ['EXPECTED_TOOL_COUNT = 150', 'generated/daily-tools-routes.json', 'public/daily-tools-data/records', 'const assets = new Map()', 'fs.writeFileSync']) if (!generator.includes(marker)) fail(`asset generator marker missing: ${marker}`);
  for (const marker of ['أدوات ومسارات مرتبطة', 'المصادر والمراجع', 'getDailyToolReferences', 'getDailyToolRelatedLinks']) if (!resources.includes(marker)) fail(`resources marker missing: ${marker}`);
  for (const marker of ['localStorage', 'rawafid:daily-tool:', 'تصدير JSON', 'window.print()']) if (!workspace.includes(marker)) fail(`workspace marker missing: ${marker}`);
  for (const marker of ['rawafid:sleep-log:v2', 'localConsent', 'تصدير CSV', 'SleepChart']) if (!sleepSource.includes(marker)) fail(`sleep tracker marker missing: ${marker}`);
  for (const marker of ['type="search"', 'useMemo', '150 أداة عملية']) if (!directory.includes(marker)) fail(`directory marker missing: ${marker}`);

  for (const marker of ["dynamic = 'force-static'", 'revalidate = false', 'await getDailyToolPage', 'dailyToolMetadata', 'deriveDailyToolDirectory']) if (!hubSource.includes(marker)) fail(`hub marker missing: ${marker}`);
  if (/getLegacyPreservedPage|force-dynamic/.test(hubSource)) fail('Daily Tools hub must remain static and repository-backed');

  for (const marker of ["dynamic = 'force-dynamic'", 'revalidate = 3600', 'await getDailyToolPage', 'dailyToolMetadata', "slug === 'sleep-wind-down-plan'", 'DailyToolWorkspace', 'DailyToolResources', 'ContentRenderer']) {
    if (!detail.includes(marker)) fail(`dynamic Daily Tools detail marker missing: ${marker}`);
  }
  if (/getLegacyPreservedPage|generateStaticParams|dynamicParams\s*=/.test(detail)) fail('Daily Tools detail pages must render directly from Cloudflare Assets without legacy lookup or SSG path generation');

  for (const marker of ["@/generated/daily-tools-routes.json", "dynamic = 'force-static'", 'EXPECTED_ROUTES = 151', 'sitemapResponse']) if (!sitemap.includes(marker)) fail(`Daily Tools sitemap marker missing: ${marker}`);
  ...
  if (!header.includes("href: '/daily-tools/'") || !header.includes("label: 'الأدوات اليومية'")) fail('global navigation must expose the Daily Tools hub');

  for (const [name, source] of Object.entries({ workspace, sleep: sleepSource, directory })) {
    if (/\bfetch\s*\(/.test(source) || /XMLHttpRequest|sendBeacon|WebSocket/.test(source)) fail(`${name} must not send personal tool input over the network`);
  }
  for (const source of [hubSource, detail]) if (/\bredirect\s*\(|permanentRedirect/.test(source)) fail('Daily Tools public routes must render in place');
}

if (bad) process.exit(1);
console.log(`Daily Tools contract passed: ${EXPECTED_TOOLS} indexable tools + hub; ${runtimeMatches} standard tools and the specialized sleep tracker retain source-derived behavior.`);
console.log('Daily Tools detail pages render on demand from immutable Cloudflare Assets; the hub and sitemap remain static and the public URL inventory is unchanged.');
