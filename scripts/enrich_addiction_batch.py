...
def build_added_blocks(spec: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for section in spec.get('sections', []):
        ...
        if title: out.append({'type': 'heading', 'level': 2, 'text': title})
        for paragraph in section.get('paragraphs', []):
            text = str(paragraph).strip()
            if text: out.append({'type': 'paragraph', 'text': text})
        items = section.get('items')
        if isinstance(items, list):
            clean = [str(item).strip() for item in items if str(item).strip()]
            if clean: out.append({'type': 'list', 'ordered': False, 'items': clean})
    faq = []
    for item in spec.get('faq', []):
        if not isinstance(item, dict): continue
        q = str(item.get('question') or '').strip(); a = str(item.get('answer') or '').strip()
        if q and a: faq.append({'question': q, 'answer': a})
    if faq:
...
def main() -> int:
    ...
    if not isinstance(records, list): raise SystemExit('invalid payload')

    specs: dict[str, Any] = {}
    version = 'addiction-core-enrichment-v1'; reviewed_at = '2026-08-09'
    ...
    for row in records:
        ...
        spec = specs.get(slug)
        if not isinstance(spec, dict): raise SystemExit(f'missing enrichment for {slug}')
        ...
        if is_condition: blocks = prune_generic_condition_sections(blocks)
        ...
        schema.update({
            ...
            'addiction_final_word_count': len(str(row['body_text']).split()),
            'addiction_reference_count': len(row['references_json']),
        })
        row['schema_json'] = schema

    payload['addiction_enrichment'] = {'version': version, 'reviewed_at': reviewed_at, 'records_enriched': len(records), 'generic_condition_sections_removed': sorted(DROP_CONDITION_H2)}
