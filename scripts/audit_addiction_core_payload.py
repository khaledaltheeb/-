...
FORBIDDEN_PHRASES = ('خطة ديتوكس منزلية','ديتوكس منزلي آمن','أوقف البنزوديازيبين فجأة','إيقاف البنزوديازيبين فجأة آمن','نتيجة مضمونة','شفاء مضمون')
EXPECTED = {
    'addiction-hub','addiction-withdrawal-safety','addiction-recovery-roadmap',
    'addiction-family-guide','addiction-sources','addiction-methodology'
}
...
def main() -> int:
    if len(sys.argv) != 2: raise SystemExit('usage: audit_addiction_core_payload.py <payload.json>')
    payload = json.loads(Path(sys.argv[1]).read_text(encoding='utf-8'))
    records = payload.get('records') if isinstance(payload, dict) else None
    if not isinstance(records, list): raise SystemExit('records array is required')
    slugs = {str(r.get('slug') or '') for r in records if isinstance(r, dict)}
    if len(records) != 6 or slugs != EXPECTED: raise SystemExit(f'expected six addiction core pages; found {sorted(slugs)}')
    ...
    for row in records:
        ...
        repeated = sum(1 for sentence in own if sum(1 for other_slug, other in sentence_sets.items() if other_slug != slug and sentence in other) >= 2)
    ...
    if failures:
        print('\nADDICTION CORE QUALITY GATE FAILED', file=sys.stderr)
        for failure in failures: print(f'- {failure}', file=sys.stderr)
        return 1
    print('\nADDICTION CORE QUALITY GATE PASSED')
    return 0
